...
gate. The default scan (`npx mjolnir-qa@4.0.0` without `--strict`)
...
<p align="center">
  <a href="assets/video/mjolnir-demo.mp4">
    <img src="assets/video/mjolnir-demo-poster.png" alt="A frame of the demo recording: npx mjolnir-qa@4.0.0 scanning the demo repository in a terminal window" width="900" />
  </a>
</p>
...
1. **Scan** — `npx mjolnir-qa@4.0.0` — scans the current directory, prints the Trust Report, exits `1` if findings at the gate
2. **Block in CI** — `npx mjolnir-qa@4.0.0 --scope changed` — scans only changed lines, blocks PRs with error findings
...
| Publishes measured false-positive rates where corpus evidence exists |             No              |       No       |       No       |       Yes        |
...
The generated workflow is advisory by default. A direct
`uses: Sergey-Bar/Mjolnir@...` composite Action is blocking by default; set
`fail-on` explicitly when its policy differs.
...
```bash
npx mjolnir-qa@4.0.0 --scope changed --blocking error
```
...
**Version 3.0.0.** The JSON schema and the exit codes are frozen contracts.

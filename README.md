...
<img src="assets/readme/hero.svg" alt="QA Doctor. Tests tell you what passed. QA Doctor tells you what you can trust." width="100%" />

<br />

QA Doctor finds tests that cannot fail and pipelines that cannot go red,<br />
...
[See it work](#see-it-work) · [Quickstart](#quickstart) · [What it finds](#what-qa-doctor-finds) · [Score](#the-test health-score) · [Evidence](#the-evidence-model) · [Forensics](#runtime-forensics) · [CI](#ci-integrity) · [Agents](#ai-agents) · [Security](#trust-and-security) · [Limits](#what-qa-doctor-cannot-tell-you) · [Docs](#documentation)
...
review. That is why they survive. Here is QA Doctor reading a real one:

<p align="center">
  <img src="assets/readme/scan.svg" alt="The demo repository's CI workflow, read line by line. QA Doctor flags each finding at the line it reported, with its rule, what is wrong, its evidence level and its measured false-positive rate." width="800" />
</p>
...
gate. The default scan (`npx qa-doctor-cli@latest` without `--strict`)
covers core and extended rules only. Add `--strict` when you want the
advisory layer too.

QA Doctor reads the suite, the CI workflows and, when you have one, the
...
| Situation                                      | What QA Doctor reports                                   |
| ---------------------------------------------- | -------------------------------------------------------- |
...
<p align="center">
  <img src="assets/readme/how-it-works.svg" alt="How QA Doctor works. It reads the test suite and the CI pipeline statically, and the report of a real run when there is one. It weighs every finding by its evidence level and its trust level, where only a real run can reach L3 to L5, and produces findings, a test health score and a CI gate on frozen exit codes. In the agent loop, AI writes the fix and QA Doctor re-scans to prove it." width="880" />
</p>
...
<p align="center">
  <img src="assets/readme/terminal-hero.svg" alt="QA Doctor's deduction breakdown: TEST HEALTH 80/100 HEALTHY, the score by category, the per-severity deduction box, and a FIX THIS FIRST list" width="520" />
</p>
...
<p align="center">
  <a href="assets/video/qa-doctor-demo.mp4">
    <img src="assets/video/qa-doctor-demo-poster.png" alt="A frame of the demo recording: npx qa-doctor-cli@latest scanning the demo repository in a terminal window" width="900" />
  </a>
</p>

<sub>Rendered frame by frame from a real scan by `npm run docs:video`;
never screen-recorded. Select the frame to open
[`qa-doctor-demo.mp4`](assets/video/qa-doctor-demo.mp4).</sub>

</details>

### One finding, up close

Every finding answers four questions: where it is, how sure QA Doctor is,
...
`qa-doctor explain QA-CI-001` prints a rule's whole trust record, including
...
WHAT WOULD CHANGE THE VERDICT
  - a run report next to the scan target (qa-doctor.report.json or
  test-results/) corroborating this file lifts its findings to L3–L5
...
`qa-doctor ci install` writes that as a GitHub Actions workflow, using the
[action](https://github.com/Sergey-Bar/qa-doctor#readme) pinned to the `v1`
major tag (or plain `npx` with `--no-action`). It stays advisory until you
decide it should block.

| Command                               | What it does                                     |
| ------------------------------------- | ------------------------------------------------ |
| `qa-doctor`                           | Trust Report: verdict, confidence, next action   |
| `qa-doctor --scope changed`           | Only what your branch introduced (the CI form)   |
| `qa-doctor ci install`                | Generate the advisory PR workflow (action-based) |
| `qa-doctor explain QA-CI-001`         | What, why and fix, plus the measured FP rate     |
| `qa-doctor why src/a.spec.ts:42`      | Why this exact line was flagged. Never gates.    |
| `qa-doctor forensics ./test-results/` | Runtime evidence from a real run                 |
| `qa-doctor trust-report`              | Self-contained Trust Artifact (md + json)        |
| `qa-doctor handoff`                   | Remediation plan for a coding agent              |
| `qa-doctor --json` / `--format sarif` | Machine-readable output, GitHub Code Scanning    |
| `qa-doctor --format codequality`      | GitLab Code Quality report (MR widget artifact)  |
...
| Command                               | What it does                                         |
| ------------------------------------- | ---------------------------------------------------- |
| `qa-doctor --classic`                 | The pre-Trust-Report score banner render             |
| `qa-doctor explain verdict`           | Why the saved scan's verdict is what it is           |
| `qa-doctor triage ./test-results/`    | Guided triage. Every row ends in a next action.      |
| `qa-doctor pw-report ./test-results/` | Playwright run summary: retries, flakes, slowest     |
| `qa-doctor doctor:playwright`         | Playwright-only deep scan plus Selector Health Score |
| `qa-doctor fix --dry-run` / `fix`     | Safe auto-fixes, each re-scanned to prove it landed  |
| `qa-doctor baseline` / `diff`         | Snapshot findings, then report only new or worse     |
| `qa-doctor impact --since <ref>`      | What a commit introduced and resolved                |
| `qa-doctor summary`                   | CI annotations and a step summary from a report      |
| `qa-doctor pr-comment`                | A scoped PR comment, as Markdown                     |
| `qa-doctor debt`                      | Test-debt register with a cost model                 |
| `qa-doctor handover`                  | Onboarding map of the suite for a new QA engineer    |
| `qa-doctor init`                      | Detect frameworks, print a setup checklist           |
| `qa-doctor suppressions`              | List suppressed findings, for governance             |
| `qa-doctor rules --unmeasured`        | The rules running on assumption, not measurement     |
| `qa-doctor rules --md`                | Full rule catalog (JSON or Markdown)                 |
| `qa-doctor doctor`                    | Self-audit of QA Doctor's own rule base              |
| `qa-doctor create-rule <ID>`          | Scaffold a new rule and its fixtures                 |
| `qa-doctor stats`                     | Local all-time counters of fixes seen                |
| `qa-doctor badge`                     | shields.io endpoint JSON and snippet                 |
| `qa-doctor --cache`                   | Incremental re-scans via a local verdict cache       |
| `qa-doctor --format mermaid`          | Test-architecture diagram for a PR comment           |

`qa-doctor help <command>` prints usage, examples and the next step for any
of them.

</details>

Requires **Node.js ≥ 22.18** on Windows, macOS or Linux. Prefer a global
install? `npm i -g qa-doctor-cli`. The floor comes from the build toolchain
(tsdown targets it and the release pipeline smoke-tests against it); the
runtime dependencies need no more than that.

<br />

## What QA Doctor finds
...
`qa-doctor rules --md`, [`docs/rules/`](docs/rules/), or the
[what-it-checks guide](https://sergey-bar.github.io/qa-doctor/guide/what-it-checks).
...
false-positive firewall; `qa-doctor doctor` enforces it in this repository's
own CI.

### Selector Health Score

`qa-doctor doctor:playwright` grades every locator by how it finds an
...
## The Test Health Score

<p align="center">
  <img src="assets/readme/score-gauge.svg" alt="The test health scale from 0 to 100, with a marker sweeping every score: CRITICAL below 50, NEEDS ATTENTION from 50 to 79, HEALTHY from 80 to 99, EXCELLENT at 100" width="720" />
</p>
...
[scoring guide](https://sergey-bar.github.io/qa-doctor/guide/scoring).
...
**none of QA Doctor's evaluated rules produced a deduction under this scan
...
Every finding carries two labels: how sure QA Doctor is, and how far the
...
reading your code. Give QA Doctor the report of a real test run and it can
...
in `qa-doctor explain`. `qa-doctor rules --unmeasured` lists them, and every
...
[rule lifecycle](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle).

### Why this is not a linter

Linters tell you whether code follows rules. QA Doctor tells you whether your
verification can be trusted.

|                                                          | Linters (ESLint, SonarQube) | Coverage tools | AI code review |  **QA Doctor**   |
| -------------------------------------------------------- | :-------------------------: | :------------: | :------------: | :--------------: |
...
can find. QA Doctor catches what AI review overlooks because it looks
...
the final checkmark says. `qa-doctor triage` turns that history into a
quarantine proposal, and `qa-doctor pw-report` summarizes a run. The same run
...
A test can pass while the pipeline around it cannot fail. QA Doctor reads the
...
**AI writes the fix. QA Doctor verifies it.** The proof comes from the
re-scan, never from the agent's own report of success.

| Command             | What the agent gets                                                                                                                                           |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa-doctor mcp`     | An [MCP](https://modelcontextprotocol.io) server over stdio. `scan`, `explain` and `diff` become callable tools.                                              |
| `qa-doctor handoff` | A saved `--json` report becomes a deterministic Markdown plan: what was detected, the evidence boundary per finding, what must **not** change, how to verify. |
| `qa-doctor install` | Writes into the agent surfaces your repo already has (`.claude/`, `.cursor/`, `.kilo/`, `AGENTS.md`) so the agent re-scans before it claims it is done.       |
...
**Plugins are not sandboxed.** JS plugins (`qa-doctor-rules/*.mjs`, or npm
...
scan**: without `--enable-plugins` (or `QA_DOCTOR_ENABLE_PLUGINS=1`) their
...
catch. `qa-doctor doctor` re-audits the rule base in the same run (fixture
...
## What QA Doctor cannot tell you
...
- **It cannot tell you an assertion is _wrong_.** `expect(total).toBe(41)`
  looks healthy. QA Doctor finds tests that _cannot fail_ and pipelines that
...
The full docs site is at <https://sergey-bar.github.io/qa-doctor/>.
...
[maturity table](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle).
What comes next, with no invented dates:
[the public roadmap](https://sergey-bar.github.io/qa-doctor/reference/roadmap).
...
[Read the guide](https://sergey-bar.github.io/qa-doctor/guide/getting-started) · [Docs site](https://sergey-bar.github.io/qa-doctor/) · [npm](https://www.npmjs.com/package/qa-doctor-cli)

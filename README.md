...
<p align="center">
  <img src="assets/readme/see-it-work.svg" alt="Mjölnir scanning a demo repo, WORTHINESS 75/100 NEEDS WORK; the CI workflow gets the exact fix the tool printed; a re-scan recovers to 90/100 WORTHY; then mjolnir handoff turns the remaining findings into a remediation plan an agent can execute" width="600" />
</p>

<p align="center">
  <strong><a href="assets/video/mjolnir-demo.mp4">▶ Prefer video?</a></strong>
  Watch the same demo at full 1440p, 42 seconds.
</p>

<sub>GitHub won't autoplay a repo-relative MP4 inline, so the SVG above is
the primary demo — same proven CSS-animation technique as the other README
assets. Every beat is real, not staged: scan/re-scan reuse the exact
captures [`tests/contract/video-script.spec.ts`](tests/contract/video-script.spec.ts)
drift-locks against actual CLI output, the fix is a real diff of two
committed workflow files (the same `diffLines()` the video renderer uses),
and the handoff excerpt is real `mjolnir handoff` output (truncated — the
full plan runs one section per finding). Regenerated by
`npm run docs:see-it-work`;
[`tests/contract/see-it-work-asset-reproducibility.spec.ts`](tests/contract/see-it-work-asset-reproducibility.spec.ts)
...
<p align="center">
  <img src="assets/readme/terminal-hero.svg" alt="Mjölnir terminal output — WORTHINESS 75/100 NEEDS WORK, a diagnostics-by-category breakdown, and a FIX THIS FIRST list" width="440" />
</p>
...
<p align="center">
  <img src="assets/readme/score-gauge.svg" alt="The hammer sweeping every score from 0 to 100 — cracked below 50 (UNWORTHY), strained 50-79 (NEEDS WORK), charged 80-99 (WORTHY), forged at 100 (FORGED) — then holding on FORGED before it loops" width="280" />
</p>

<sub>Regenerated by `npm run docs:gauge`;
[`tests/contract/score-gauge-asset-reproducibility.spec.ts`](tests/contract/score-gauge-asset-reproducibility.spec.ts)
fails CI if it drifts from what `deriveScoreState`/`renderHammer`/`scoreGauge`
actually produce. The hammer only has four real states — it snaps between
them at the exact thresholds above, it doesn't fake a smooth in-between
shape that doesn't exist in the tool.</sub>

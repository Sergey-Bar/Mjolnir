...
<img src="assets/readme/hero.svg" alt="QA Doctor เทสต์บอกคุณว่าอะไรผ่าน QA Doctor บอกคุณว่าอะไรเชื่อถือได้" width="100%" />

<br />

QA Doctor ค้นหาเทสต์ที่ไม่มีวันล้มเหลวและไปป์ไลน์ที่ไม่มีวันเป็นสีแดง<br />
...
[ดูการทำงานจริง](#ดูการทำงานจริง) · [เริ่มต้นอย่างรวดเร็ว](#เริ่มต้นอย่างรวดเร็ว) · [สิ่งที่ตรวจพบ](#สิ่งที่-qa-doctor-ตรวจพบ) · [คะแนน](#คะแนนความน่าเชื่อถือ) · [หลักฐาน](#แบบจำลองหลักฐาน) · [นิติวิเคราะห์การรัน](#นิติวิเคราะห์ขณะรัน) · [CI](#ความสมบูรณ์ของ-ci) · [เอเจนต์](#เอเจนต์-ai) · [ความปลอดภัย](#ความเชื่อถือและความปลอดภัย) · [ข้อจำกัด](#สิ่งที่-qa-doctor-บอกคุณไม่ได้) · [เอกสาร](#เอกสารประกอบ)
...
ไม่มีข้อใดทำให้ไปป์ไลน์เป็นสีแดง และทุกข้อดูเหมือนตั้งใจเมื่ออยู่ในการรีวิว นั่นคือเหตุผลที่มันรอดมาได้ นี่คือ QA Doctor กำลังอ่านกรณีจริง:

<p align="center">
  <img src="assets/readme/scan.svg" alt="CI workflow ของรีโพสาธิต อ่านทีละบรรทัด QA Doctor ทำเครื่องหมายแต่ละข้อค้นพบที่บรรทัดที่รายงาน พร้อมกฎ สิ่งที่ผิด ระดับหลักฐาน และอัตราผลบวกลวงที่วัดได้" width="800" />
</p>
...
**โหมดเข้มงวด.** การตรวจจับที่รุนแรงที่สุด — `.only`, `continue-on-error`, การทดสอบว่าง, การใช้ retry ในทางที่ผิด — อยู่ในชั้นกักกัน ทำงานเฉพาะภายใต้ `--strict` และจำกัดที่ความรุนแรง `info`: ตั้งค่าสถานะ แต่ไม่เคยบล็อก การสแกนเริ่มต้น (`npx qa-doctor-cli@latest` โดยไม่มี `--strict`) ครอบคลุมเฉพาะกฎหลักและกฎขยาย เพิ่ม `--strict` เมื่อคุณต้องการชั้นที่ปรึกษาด้วย

QA Doctor อ่านชุดเทสต์ CI workflow และรายงานจากการรันจริงหากคุณมี มันไม่รันเทสต์ของคุณ ไม่ติดตั้ง dependency และไม่รันโค้ดที่มันสแกน และเมื่อไม่มีหลักฐาน มันจะบอกตรง ๆ แทนที่จะแต่งความมั่นใจขึ้นมา:

| สถานการณ์                                         | สิ่งที่ QA Doctor รายงาน                                       |
| ------------------------------------------------- | -------------------------------------------------------------- |
...
<p align="center">
  <img src="assets/readme/how-it-works.svg" alt="QA Doctor ทำงานอย่างไร มันอ่านชุดเทสต์และไปป์ไลน์ CI แบบสถิต และอ่านรายงานจากการรันจริงเมื่อมี มันถ่วงน้ำหนักทุกข้อค้นพบตามระดับหลักฐานและระดับความเชื่อถือ โดยมีเพียงการรันจริงเท่านั้นที่ไปถึง L3 ถึง L5 ได้ แล้วสร้างข้อค้นพบ คะแนนความน่าเชื่อถือ และด่าน CI ที่ใช้รหัสออกซึ่งถูกตรึงไว้ ในลูปของเอเจนต์ AI เขียนการแก้ไข แล้ว QA Doctor สแกนซ้ำเพื่อพิสูจน์" width="880" />
</p>
...
<p align="center">
  <a href="assets/video/qa-doctor-demo.mp4">
    <img src="assets/video/qa-doctor-demo-poster.png" alt="เฟรมหนึ่งจากวิดีโอสาธิต: npx qa-doctor-cli@latest กำลังสแกนรีโพสาธิตในหน้าต่างเทอร์มินัล" width="900" />
  </a>
</p>

<sub>เรนเดอร์ทีละเฟรมจากการสแกนจริงด้วย `npm run docs:video` ไม่เคยอัดหน้าจอ เลือกเฟรมเพื่อเปิด [`qa-doctor-demo.mp4`](assets/video/qa-doctor-demo.mp4)</sub>

</details>

### ข้อค้นพบหนึ่งข้อแบบใกล้ชิด

ทุกข้อค้นพบตอบคำถามสี่ข้อ: อยู่ที่ไหน QA Doctor มั่นใจแค่ไหน กฎนี้ผิดบ่อยแค่ไหน และแก้อย่างไร
...
`qa-doctor explain QA-CI-001` พิมพ์ประวัติความน่าเชื่อถือทั้งหมดของกฎ รวมถึงอัตราผลบวกลวงที่วัดได้และระดับที่อัตรานั้นทำให้กฎได้รับ:
...
`qa-doctor ci install` เขียนสิ่งนี้เป็น GitHub Actions workflow โดยใช้ [action](https://github.com/Sergey-Bar/qa-doctor#readme) ที่ปักไว้กับแท็กหลัก `v1` (หรือใช้ `npx` ธรรมดาด้วย `--no-action`) มันจะอยู่ในโหมดให้คำแนะนำจนกว่าคุณจะตัดสินใจให้มันบล็อก

| คำสั่ง                                | สิ่งที่ทำ                                                |
| ------------------------------------- | -------------------------------------------------------- |
| `qa-doctor`                           | Trust Report: คำตัดสิน ความมั่นใจ ขั้นตอนถัดไป           |
| `qa-doctor --scope changed`           | เฉพาะสิ่งที่ branch ของคุณนำเข้ามา (รูปแบบสำหรับ CI)     |
| `qa-doctor ci install`                | สร้าง PR workflow แบบให้คำแนะนำ (ใช้ action)             |
| `qa-doctor explain QA-CI-001`         | อะไร ทำไม และวิธีแก้ พร้อมอัตรา FP ที่วัดได้             |
| `qa-doctor why src/a.spec.ts:42`      | เหตุผลที่บรรทัดนี้ถูกทำเครื่องหมาย ไม่บล็อกเลย           |
| `qa-doctor forensics ./test-results/` | หลักฐานรันไทม์จากการรันจริง                              |
| `qa-doctor trust-report`              | Trust Artifact แบบครบในตัว (md + json)                   |
| `qa-doctor handoff`                   | แผนการแก้ไขสำหรับเอเจนต์เขียนโค้ด                        |
| `qa-doctor --json` / `--format sarif` | ผลลัพธ์ที่เครื่องอ่านได้, GitHub Code Scanning           |
| `qa-doctor --format codequality`      | รายงาน GitLab Code Quality (อาร์ติแฟกต์สำหรับวิดเจ็ต MR) |
| `qa-doctor --strict`                  | รันกฎระดับ quarantine ด้วย (ความเสี่ยง FP สูงกว่า)       |
...
| คำสั่ง                                | สิ่งที่ทำ                                                             |
| ------------------------------------- | --------------------------------------------------------------------- |
| `qa-doctor --classic`                 | แบนเนอร์คะแนนแบบก่อนมี Trust Report                                   |
| `qa-doctor explain verdict`           | เหตุผลที่คำตัดสินของการสแกนที่บันทึกไว้เป็นเช่นนั้น                   |
| `qa-doctor triage ./test-results/`    | การคัดแยกแบบมีขั้นตอน ทุกแถวจบด้วยขั้นตอนถัดไป                        |
| `qa-doctor pw-report ./test-results/` | สรุปการรัน Playwright: การรันซ้ำ เทสต์ไม่เสถียร เทสต์ที่ช้าที่สุด     |
| `qa-doctor doctor:playwright`         | สแกนเชิงลึกเฉพาะ Playwright พร้อม Selector Health Score               |
| `qa-doctor fix --dry-run` / `fix`     | การแก้ไขอัตโนมัติที่ปลอดภัย แต่ละรายการถูกสแกนซ้ำเพื่อพิสูจน์ว่าได้ผล |
| `qa-doctor baseline` / `diff`         | บันทึกสแนปช็อตของข้อค้นพบ แล้วรายงานเฉพาะที่ใหม่หรือแย่ลง             |
| `qa-doctor impact --since <ref>`      | สิ่งที่คอมมิตหนึ่งนำเข้ามาและแก้ไขไป                                  |
| `qa-doctor summary`                   | คำอธิบายประกอบ CI และสรุป step จากรายงาน                              |
| `qa-doctor pr-comment`                | คอมเมนต์ PR ที่จำกัดขอบเขต เป็น Markdown                              |
| `qa-doctor debt`                      | ทะเบียนหนี้ทางเทสต์พร้อมแบบจำลองต้นทุน                                |
| `qa-doctor handover`                  | แผนที่แนะนำชุดเทสต์สำหรับวิศวกร QA คนใหม่                             |
| `qa-doctor init`                      | ตรวจหาเฟรมเวิร์กและพิมพ์รายการตรวจสอบการตั้งค่า                       |
| `qa-doctor suppressions`              | แสดงรายการข้อค้นพบที่ถูกระงับ เพื่อการกำกับดูแล                       |
| `qa-doctor rules --unmeasured`        | กฎที่ทำงานจากการสันนิษฐาน ไม่ใช่จากการวัด                             |
| `qa-doctor rules --md`                | แค็ตตาล็อกกฎฉบับเต็ม (JSON หรือ Markdown)                             |
| `qa-doctor doctor`                    | การตรวจสอบตัวเองของฐานกฎของ QA Doctor                                 |
| `qa-doctor create-rule <ID>`          | สร้างโครงของกฎใหม่และ fixture ของมัน                                  |
| `qa-doctor stats`                     | ตัวนับสะสมในเครื่องของการแก้ไขทั้งหมดที่เคยพบ                         |
| `qa-doctor badge`                     | JSON สำหรับ endpoint ของ shields.io และโค้ดตัวอย่าง                   |
| `qa-doctor --cache`                   | สแกนซ้ำแบบเพิ่มทีละส่วนผ่านแคชคำตัดสินในเครื่อง                       |
| `qa-doctor --format mermaid`          | แผนภาพสถาปัตยกรรมเทสต์สำหรับคอมเมนต์ PR                               |

`qa-doctor help <command>` พิมพ์วิธีใช้ ตัวอย่าง และขั้นตอนถัดไปของทุกคำสั่ง

</details>

ต้องใช้ **Node.js ≥ 22.18** บน Windows, macOS หรือ Linux อยากติดตั้งแบบ global หรือไม่? `npm i -g qa-doctor-cli` เวอร์ชันขั้นต่ำนี้มาจากชุดเครื่องมือ build (tsdown กำหนดเป้าหมายไว้ที่เวอร์ชันนี้ และไปป์ไลน์รีลีสทำ smoke test กับเวอร์ชันนี้) dependency ตอนรันไม่ต้องการมากกว่านั้น

<br />

## สิ่งที่ QA Doctor ตรวจพบ
...
แค็ตตาล็อกฉบับเต็มสร้างจากทะเบียน ไม่เคยดูแลด้วยมือ: `qa-doctor rules --md`, [`docs/rules/`](docs/rules/) หรือ [คู่มือสิ่งที่ตรวจสอบ](https://sergey-bar.github.io/qa-doctor/guide/what-it-checks)
...
ทุกกฎมาพร้อม fixture แบบ must-fire **และ** must-not-fire และกฎที่ทำงานกับ fixture เชิงลบของตัวเองจะไม่สามารถปล่อยได้ นี่คือไฟร์วอลล์กันผลบวกลวง `qa-doctor doctor` บังคับใช้สิ่งนี้ใน CI ของรีโพนี้เอง

### Selector Health Score

`qa-doctor doctor:playwright` ให้คะแนน locator แต่ละตัวตามวิธีที่มันหาองค์ประกอบ: แบบที่ผู้ใช้หา (บทบาท ป้ายกำกับ ข้อความ) ผ่านสัญญาที่ชัดเจน (`data-testid`) หรือด้วยความบังเอิญเชิงโครงสร้าง (สาย CSS, XPath) แต่ละไฟล์ได้คะแนนตั้งแต่ 0 ถึง 100:
...
**สิ่งที่ 100 ไม่ได้หมายถึง** ไม่ได้หมายความว่าซอฟต์แวร์ถูกต้อง ชุดเทสต์เพียงพอ หรือผลิตภัณฑ์ไม่มีข้อบกพร่อง มันหมายถึงเพียงสิ่งเดียว: **ไม่มีกฎใดที่ QA Doctor ประเมินทำให้เกิดการหักคะแนนภายใต้การสแกนนี้และแบบจำลองหลักฐานนี้**

<br />

## แบบจำลองหลักฐาน

ทุกข้อค้นพบมีป้ายกำกับสองอย่าง: QA Doctor มั่นใจแค่ไหน และข้อค้นพบถูกตรวจสอบไปไกลแค่ไหน นี่คือความต่างระหว่างเครื่องมือที่รายงานรูปแบบ กับเครื่องมือที่คุณใช้เป็นด่านก่อนปล่อยรีลีสได้
...
**ตรวจสอบไปไกลแค่ไหน — ระดับความเชื่อถือ** ข้อค้นพบส่วนใหญ่มาจากการอ่านโค้ดของคุณ ให้รายงานจากการรันเทสต์จริงแก่ QA Doctor แล้วมันจะยืนยันได้ว่าโค้ดถูกรันจริง
...
**74 จาก 79 กฎมีอัตราผลบวกลวงที่วัดกับโค้ด OSS จริง** (อย่างน้อย 10 ข้อค้นพบที่จัดประเภทด้วยมือต่อกฎ ดู [docs/FP-AUDIT.md](docs/FP-AUDIT.md)) อีก 5 กฎปล่อยออกมาด้วยค่าประมาณของผู้เขียนและบอกไว้ชัดเจนทีละกฎใน `qa-doctor explain` `qa-doctor rules --unmeasured` แสดงรายการกฎเหล่านั้น และท้ายการสแกนทุกครั้งจะรายงานว่ามีกฎกี่ข้อในบรรดาที่ _ทำงานจริง_ ที่ผ่านการวัดแล้ว
...
การเลื่อนระดับ การลดระดับ และความพร้อมรายภาษา: [วงจรชีวิตของกฎ](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle)

### ทำไมนี่จึงไม่ใช่ linter

Linter บอกคุณว่าโค้ดทำตามกฎหรือไม่ QA Doctor บอกคุณว่าการตรวจสอบของคุณเชื่อถือได้หรือไม่

|                                                               | Linter (ESLint, SonarQube) | เครื่องมือวัด coverage | รีวิวโค้ดด้วย AI |     **QA Doctor**     |
| ------------------------------------------------------------- | :------------------------: | :--------------------: | :--------------: | :-------------------: |
...
ใช้การรีวิวด้วย AI ด้วย มันจับรายละเอียดปลีกย่อย เจตนา และข้อบกพร่องด้านการออกแบบที่ไม่มีรูปแบบใดหาเจอ ส่วน QA Doctor จับสิ่งที่การรีวิวด้วย AI มองข้ามเพราะดูเหมือนตั้งใจ: `.only` ที่ถูกคอมมิต รหัสออกที่ถูกกลืน `continue-on-error` บน job เทสต์ สิ่งเหล่านี้ต้องการการสแกน ไม่ใช่การใช้เหตุผล
...
`TRUE-FLAKE` ไม่ได้หมายความว่าเทสต์ถูกรันซ้ำ แต่หมายความว่าเทสต์ **ล้มเหลวอย่างน้อยหนึ่งครั้งแล้วจบลงเป็นสีเขียว** คือการผ่านแบบฟลุก ซึ่งจะถูกทำเครื่องหมายไม่ว่าเครื่องหมายถูกสุดท้ายจะบอกอะไร `qa-doctor triage` เปลี่ยนประวัตินั้นเป็นข้อเสนอให้กักกัน และ `qa-doctor pw-report` สรุปการรัน รายงานการรันชุดเดียวกันนี้คือสิ่งที่ยกข้อค้นพบขึ้นสู่ระดับความเชื่อถือ L3 ขึ้นไป

<br />

## ความสมบูรณ์ของ CI

เทสต์หนึ่งอาจผ่าน ขณะที่ไปป์ไลน์รอบตัวมันไม่มีทางล้มเหลวได้ QA Doctor อ่าน workflow ด้วย: `continue-on-error`, `|| true` รหัสออกที่ไม่เคยถูกส่งต่อ step ที่สำเร็จเสมอ รายงานที่ถูกใช้แต่ไม่เคยถูกสร้าง และด่านที่ถูกข้ามในเหตุการณ์ที่ควรบล็อก แต่ละข้อค้นพบระบุ job, step และบรรทัด พร้อมระดับหลักฐานของตัวเอง
...
**AI เขียนการแก้ไข QA Doctor ตรวจสอบมัน** ข้อพิสูจน์มาจากการสแกนซ้ำ ไม่ใช่จากรายงานความสำเร็จของเอเจนต์เอง

| คำสั่ง              | สิ่งที่เอเจนต์ได้รับ                                                                                                                                 |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa-doctor mcp`     | เซิร์ฟเวอร์ [MCP](https://modelcontextprotocol.io) ผ่าน stdio `scan`, `explain` และ `diff` กลายเป็นเครื่องมือที่เรียกใช้ได้                          |
| `qa-doctor handoff` | รายงาน `--json` ที่บันทึกไว้กลายเป็นแผน Markdown แบบกำหนดแน่นอน: สิ่งที่ตรวจพบ ขอบเขตหลักฐานของแต่ละข้อค้นพบ สิ่งที่ **ห้าม** เปลี่ยน และวิธีตรวจสอบ |
| `qa-doctor install` | เขียนลงในพื้นที่สำหรับเอเจนต์ที่รีโพของคุณมีอยู่แล้ว (`.claude/`, `.cursor/`, `.kilo/`, `AGENTS.md`) เพื่อให้เอเจนต์สแกนซ้ำก่อนจะบอกว่าทำเสร็จแล้ว   |
...
**ปลั๊กอินไม่ได้รันใน sandbox** ปลั๊กอิน JS (`qa-doctor-rules/*.mjs` หรือแพ็กเกจ npm ที่ระบุไว้ใต้ `"plugins"`) รันด้วยสิทธิ์ Node เต็มรูปแบบ ซึ่งเป็นแบบจำลองความเชื่อถือเดียวกับปลั๊กอินของ ESLint หรือ Vitest การโหลดปลั๊กอินต้องเลือกเปิดเอง **ในทุกการสแกน**: หากไม่มี `--enable-plugins` (หรือ `QA_DOCTOR_ENABLE_PLUGINS=1`) ซอร์สของปลั๊กอินจะไม่ถูกโหลดเลย และประกาศบน stderr จะแสดงรายการสิ่งที่ถูกข้าม manifest ของกฎที่เป็น JSON ไม่รันโค้ดใด ๆ และคำนำหน้า ID ของกฎ core ถูกสงวนไว้ เพื่อไม่ให้ปลั๊กอินปลอมตัวเป็นกฎเหล่านั้นได้ รายงานช่องโหว่ผ่าน [SECURITY.md](SECURITY.md)

**มันรันกับตัวเอง** เอนจินความเชื่อถือในการตรวจสอบจะไม่มีความน่าเชื่อถือเลย หากตัวมันเองตรวจสอบไม่ได้ ทุกการรัน CI สแกนรีโพนี้ด้วย build ที่การรันเดียวกันนั้นสร้างขึ้น ด่านจะล้มเหลวเมื่อพบข้อค้นพบระดับ error ใด ๆ และเมื่อการสแกนเป็นแบบ **บางส่วน** หรือมี **กฎที่พัง** เพราะการสแกนตัวเองที่ถูกตัดจบและไม่รายงานอะไรเลย ก็คือสีเขียวปลอมที่โปรเจกต์นี้มีไว้จับ `qa-doctor doctor` ตรวจสอบฐานกฎซ้ำในการรันเดียวกัน (ไฟร์วอลล์ fixture ความซื่อตรงของระดับ เพดานของระดับ core) และการตรวจที่ได้ผล INCONCLUSIVE จะล้มเหลวเหมือนกับการตรวจที่ล้มเหลวทุกประการ รายงานทั้งสองถูกอัปโหลดเป็นอาร์ติแฟกต์ของ build
...
## สิ่งที่ QA Doctor บอกคุณไม่ได้

- **มันไม่รันเทสต์ของคุณ** การสแกนที่สะอาดไม่ใช่ชุดเทสต์ที่ผ่าน
- **มันบอกไม่ได้ว่า assertion _ผิด_** `expect(total).toBe(41)` ดูปกติดี QA Doctor หาเทสต์ที่ _ล้มเหลวไม่ได้_ และไปป์ไลน์ที่ _เป็นสีแดงไม่ได้_ ไม่ใช่เทสต์ที่ตรวจสอบผิดเรื่อง
...
เว็บไซต์เอกสารฉบับเต็มอยู่ที่ <https://sergey-bar.github.io/qa-doctor/>
...
**เวอร์ชัน 1** schema ของ JSON และรหัสออกเป็นสัญญาที่ถูกตรึงไว้ TypeScript และ Python มีการครอบคลุมที่วัดได้กว้างที่สุด Java และ C# ใหม่กว่า ให้อ่านผ่าน [ตารางความพร้อม](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle) สิ่งที่จะมาต่อไป โดยไม่มีวันที่ที่แต่งขึ้น: [แผนงานสาธารณะ](https://sergey-bar.github.io/qa-doctor/reference/roadmap)
...
[อ่านคู่มือ](https://sergey-bar.github.io/qa-doctor/guide/getting-started) · [เว็บไซต์เอกสาร](https://sergey-bar.github.io/qa-doctor/) · [npm](https://www.npmjs.com/package/qa-doctor-cli)

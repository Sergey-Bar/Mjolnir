...
[Zobacz w działaniu](#zobacz-w-działaniu) · [Szybki start](#szybki-start) · [Co znajduje](#co-znajduje-qa-doctor) · [Wynik](#wynik-wiarygodności) · [Dowody](#model-dowodów) · [Analiza przebiegów](#analiza-przebiegów-testów) · [CI](#integralność-ci) · [Agenci](#agenci-ai) · [Bezpieczeństwo](#zaufanie-i-bezpieczeństwo) · [Ograniczenia](#czego-qa-doctor-nie-może-ci-powiedzieć) · [Dokumentacja](#dokumentacja)
...
<p align="center">
  <a href="assets/video/qa-doctor-demo.mp4">
    <img src="assets/video/qa-doctor-demo-poster.png" alt="Klatka nagrania demonstracyjnego: npx qa-doctor-cli@latest skanuje repozytorium demonstracyjne w oknie terminala" width="900" />
  </a>
</p>

<sub>Wyrenderowane klatka po klatce z prawdziwego skanu przez `npm run docs:video`; nigdy nagrywane z ekranu. Wybierz klatkę, aby otworzyć [`qa-doctor-demo.mp4`](assets/video/qa-doctor-demo.mp4).</sub>
...
`qa-doctor explain QA-CI-001` wypisuje całą kartotekę zaufania reguły, w tym zmierzony odsetek fałszywych alarmów i poziom, który ten odsetek jej zapewnił:
...
`qa-doctor ci install` zapisuje to jako workflow GitHub Actions, używając [akcji](https://github.com/Sergey-Bar/qa-doctor#readme) przypiętej do głównego tagu `v1` (albo zwykłego `npx` z `--no-action`). Pozostaje doradczy, dopóki nie zdecydujesz, że ma blokować.

| Polecenie                             | Co robi                                                     |
| ------------------------------------- | ----------------------------------------------------------- |
| `qa-doctor`                           | Trust Report: werdykt, pewność, następny krok               |
| `qa-doctor --scope changed`           | Tylko to, co wprowadziła twoja gałąź (forma dla CI)         |
| `qa-doctor ci install`                | Generuje doradczy workflow dla PR (oparty na akcji)         |
| `qa-doctor explain QA-CI-001`         | Co, dlaczego i jak naprawić, plus zmierzony odsetek FP      |
| `qa-doctor why src/a.spec.ts:42`      | Dlaczego oznaczono dokładnie ten wiersz. Nigdy nie blokuje. |
| `qa-doctor forensics ./test-results/` | Dowody z prawdziwego przebiegu                              |
| `qa-doctor trust-report`              | Samodzielny Trust Artifact (md + json)                      |
| `qa-doctor handoff`                   | Plan naprawy dla agenta kodującego                          |
| `qa-doctor --json` / `--format sarif` | Wyjście czytelne maszynowo, GitHub Code Scanning            |
| `qa-doctor --format codequality`      | Raport GitLab Code Quality (artefakt widżetu MR)            |
| `qa-doctor --strict`                  | Uruchamia też reguły poziomu quarantine (wyższe ryzyko FP)  |
...
| Polecenie                             | Co robi                                                                                 |
| ------------------------------------- | --------------------------------------------------------------------------------------- |
| `qa-doctor --classic`                 | Baner wyniku sprzed Trust Reportu                                                       |
| `qa-doctor explain verdict`           | Dlaczego werdykt zapisanego skanu jest taki, a nie inny                                 |
| `qa-doctor triage ./test-results/`    | Prowadzony triaż. Każdy wiersz kończy się następnym krokiem.                            |
| `qa-doctor pw-report ./test-results/` | Podsumowanie przebiegu Playwright: ponowienia, niestabilne testy, najwolniejsze         |
| `qa-doctor doctor:playwright`         | Głęboki skan tylko dla Playwright plus Selector Health Score                            |
| `qa-doctor fix --dry-run` / `fix`     | Bezpieczne automatyczne poprawki, każda ponownie skanowana, by udowodnić, że zadziałała |
| `qa-doctor baseline` / `diff`         | Zapisuje stan znalezisk, a potem zgłasza tylko nowe lub gorsze                          |
| `qa-doctor impact --since <ref>`      | Co commit wprowadził i rozwiązał                                                        |
| `qa-doctor summary`                   | Adnotacje CI i podsumowanie kroku na podstawie raportu                                  |
| `qa-doctor pr-comment`                | Komentarz do PR o ograniczonym zakresie, w Markdown                                     |
| `qa-doctor debt`                      | Rejestr długu testowego z modelem kosztów                                               |
| `qa-doctor handover`                  | Mapa wdrożeniowa zestawu dla nowego inżyniera QA                                        |
| `qa-doctor init`                      | Wykrywa frameworki, wypisuje listę kontrolną konfiguracji                               |
| `qa-doctor suppressions`              | Wyświetla wyciszone znaleziska, na potrzeby nadzoru                                     |
| `qa-doctor rules --unmeasured`        | Reguły działające na założeniu, a nie na pomiarze                                       |
| `qa-doctor rules --md`                | Pełny katalog reguł (JSON lub Markdown)                                                 |
| `qa-doctor doctor`                    | Autoaudyt własnej bazy reguł QA Doctor                                                  |
| `qa-doctor create-rule <ID>`          | Tworzy szkielet nowej reguły i jej fixture'ów                                           |
| `qa-doctor stats`                     | Lokalne liczniki wszystkich widzianych poprawek                                         |
| `qa-doctor badge`                     | JSON endpointu shields.io i fragment kodu                                               |
| `qa-doctor --cache`                   | Przyrostowe ponowne skany dzięki lokalnej pamięci podręcznej werdyktów                  |
| `qa-doctor --format mermaid`          | Diagram architektury testów do komentarza w PR                                          |

`qa-doctor help <command>` wypisuje sposób użycia, przykłady i następny krok dla każdego z nich.

</details>

Wymaga **Node.js ≥ 22.18** w systemie Windows, macOS lub Linux. Wolisz instalację globalną? `npm i -g qa-doctor-cli`. Minimalna wersja wynika z łańcucha budowania (tsdown ją obsługuje, a pipeline wydań wykonuje na niej testy dymne); zależności uruchomieniowe nie potrzebują nic więcej.
...
Pełny katalog jest generowany z rejestru, nigdy nie jest utrzymywany ręcznie: `qa-doctor rules --md`, [`docs/rules/`](docs/rules/) albo [przewodnik po tym, co sprawdza](https://sergey-bar.github.io/qa-doctor/guide/what-it-checks).
...
Każda reguła trafia do wydania z fixture'em must-fire **i** must-not-fire, a reguła, która odpala na własnym negatywnym fixture'ze, nie może zostać wydana. To zapora przed fałszywymi alarmami; `qa-doctor doctor` egzekwuje ją we własnym CI tego repozytorium.

### Selector Health Score

`qa-doctor doctor:playwright` ocenia każdy lokator według tego, jak znajduje element: tak jak zrobiłby to użytkownik (rola, etykieta, tekst), przez jawny kontrakt (`data-testid`) lub przez przypadek strukturalny (łańcuchy CSS, XPath). Każdy plik dostaje wynik od 0 do 100:
...
**Czego 100 nie oznacza.** Nie oznacza, że oprogramowanie jest poprawne, zestaw testów wystarczający, a produkt wolny od defektów. Oznacza jedno: **żadna z reguł ocenionych przez QA Doctor nie dała potrącenia w tym skanie i przy tym modelu dowodów.**
...
**Jak daleko sprawdzone — poziom zaufania.** Większość znalezisk pochodzi z czytania twojego kodu. Daj QA Doctor raport z prawdziwego przebiegu testów, a potwierdzi, że kod naprawdę się wykonał.
...
**74 z 79 reguł ma odsetek fałszywych alarmów zmierzony na prawdziwym kodzie OSS** (co najmniej 10 ręcznie sklasyfikowanych znalezisk każda; zobacz [docs/FP-AUDIT.md](docs/FP-AUDIT.md)). Pozostałe 5 opiera się na szacunku autora i mówi to, reguła po regule, w `qa-doctor explain`. `qa-doctor rules --unmeasured` je wypisuje, a stopka każdego skanu podaje, ile z reguł, które faktycznie _odpaliły_, jest zmierzonych.
...
Awans, degradacja i dojrzałość według języka: [cykl życia reguł](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle).
...
`TRUE-FLAKE` nie oznacza, że test był ponawiany. Oznacza, że test **zawiódł przynajmniej jedną próbę, a potem zakończył się na zielono**: szczęśliwe przejście, oznaczone bez względu na to, co mówi końcowy znacznik. `qa-doctor triage` zamienia tę historię w propozycję kwarantanny, a `qa-doctor pw-report` podsumowuje przebieg. To te same raporty z przebiegów podnoszą znaleziska do poziomów zaufania L3 i wyżej.
...
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa-doctor mcp`     | Serwer [MCP](https://modelcontextprotocol.io) przez stdio. `scan`, `explain` i `diff` stają się narzędziami do wywołania.                                                    |
| `qa-doctor handoff` | Zapisany raport `--json` staje się deterministycznym planem w Markdown: co wykryto, granica dowodu dla każdego znaleziska, co **nie** może się zmienić, jak to zweryfikować. |
| `qa-doctor install` | Zapisuje w miejscach dla agentów, które twoje repo już ma (`.claude/`, `.cursor/`, `.kilo/`, `AGENTS.md`), aby agent skanował ponownie, zanim oświadczy, że skończył.        |
...
Pełna strona dokumentacji jest pod adresem <https://sergey-bar.github.io/qa-doctor/>.
...
**Wersja 1.** Schemat JSON i kody wyjścia są zamrożonymi kontraktami. TypeScript i Python mają najszersze zmierzone pokrycie. Java i C# są nowsze; czytaj je przez [tabelę dojrzałości](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle). Co dalej, bez wymyślonych dat: [publiczna mapa drogowa](https://sergey-bar.github.io/qa-doctor/reference/roadmap).
...
[Przeczytaj przewodnik](https://sergey-bar.github.io/qa-doctor/guide/getting-started) · [Strona dokumentacji](https://sergey-bar.github.io/qa-doctor/) · [npm](https://www.npmjs.com/package/qa-doctor-cli)

...
<p align="center">
  <a href="assets/video/qa-doctor-demo.mp4">
    <img src="assets/video/qa-doctor-demo-poster.png" alt="Ein Frame der Demo-Aufnahme: npx qa-doctor-cli@latest scannt das Demo-Repository in einem Terminalfenster" width="900" />
  </a>
</p>

<sub>Frame für Frame aus einem echten Scan gerendert mit `npm run docs:video`; nie per Bildschirmaufnahme. Wähle den Frame, um [`qa-doctor-demo.mp4`](assets/video/qa-doctor-demo.mp4) zu öffnen.</sub>
...
`qa-doctor explain QA-CI-001` gibt die gesamte Vertrauensakte einer Regel aus, einschließlich ihrer gemessenen Falsch-Positiv-Rate und der Stufe, die ihr diese Rate eingebracht hat:
...
`qa-doctor ci install` schreibt das als GitHub-Actions-Workflow, mit der [Action](https://github.com/Sergey-Bar/qa-doctor#readme), gepinnt auf das Major-Tag `v1` (oder schlichtes `npx` mit `--no-action`). Er bleibt beratend, bis du entscheidest, dass er blockieren soll.

| Befehl                                | Was er tut                                                     |
| ------------------------------------- | -------------------------------------------------------------- |
| `qa-doctor`                           | Trust Report: Urteil, Konfidenz, nächster Schritt              |
| `qa-doctor --scope changed`           | Nur was dein Branch eingeführt hat (die CI-Form)               |
| `qa-doctor ci install`                | Beratenden PR-Workflow erzeugen (Action-basiert)               |
| `qa-doctor explain QA-CI-001`         | Was, warum und Fix, plus die gemessene FP-Rate                 |
| `qa-doctor why src/a.spec.ts:42`      | Warum genau diese Zeile markiert wurde. Blockiert nie.         |
| `qa-doctor forensics ./test-results/` | Laufzeit-Evidenz aus einem echten Lauf                         |
| `qa-doctor trust-report`              | Eigenständiges Trust-Artefakt (md + json)                      |
| `qa-doctor handoff`                   | Behebungsplan für einen Coding-Agenten                         |
| `qa-doctor --json` / `--format sarif` | Maschinenlesbare Ausgabe, GitHub Code Scanning                 |
| `qa-doctor --format codequality`      | GitLab-Code-Quality-Report (MR-Widget-Artefakt)                |
| `qa-doctor --strict`                  | Auch Regeln der Stufe quarantine ausführen (höheres FP-Risiko) |
...
| Befehl                                | Was er tut                                                                       |
| ------------------------------------- | -------------------------------------------------------------------------------- |
| `qa-doctor --classic`                 | Das Score-Banner aus der Zeit vor dem Trust Report                               |
| `qa-doctor explain verdict`           | Warum das Urteil des gespeicherten Scans so ist, wie es ist                      |
| `qa-doctor triage ./test-results/`    | Geführte Triage. Jede Zeile endet mit einem nächsten Schritt.                    |
| `qa-doctor pw-report ./test-results/` | Playwright-Laufzusammenfassung: Retries, Flakes, langsamste Tests                |
| `qa-doctor doctor:playwright`         | Tiefenscan nur für Playwright plus Selector Health Score                         |
| `qa-doctor fix --dry-run` / `fix`     | Sichere Auto-Fixes, jeder erneut gescannt, um zu beweisen, dass er gegriffen hat |
| `qa-doctor baseline` / `diff`         | Befunde als Snapshot sichern, dann nur neue oder schlimmere melden               |
| `qa-doctor impact --since <ref>`      | Was ein Commit eingeführt und behoben hat                                        |
| `qa-doctor summary`                   | CI-Annotationen und eine Step-Zusammenfassung aus einem Report                   |
| `qa-doctor pr-comment`                | Ein auf den PR zugeschnittener Kommentar, als Markdown                           |
| `qa-doctor debt`                      | Testschulden-Register mit Kostenmodell                                           |
| `qa-doctor handover`                  | Einarbeitungskarte der Suite für neue QA-Engineers                               |
| `qa-doctor init`                      | Frameworks erkennen, Setup-Checkliste ausgeben                                   |
| `qa-doctor suppressions`              | Unterdrückte Befunde auflisten, für die Governance                               |
| `qa-doctor rules --unmeasured`        | Die Regeln, die auf Annahme statt Messung laufen                                 |
| `qa-doctor rules --md`                | Vollständiger Regelkatalog (JSON oder Markdown)                                  |
| `qa-doctor doctor`                    | Selbstaudit von QA Doctor eigener Regelbasis                                     |
| `qa-doctor create-rule <ID>`          | Gerüst für eine neue Regel und ihre Fixtures                                     |
| `qa-doctor stats`                     | Lokale Zähler aller jemals gesehenen Fixes                                       |
| `qa-doctor badge`                     | shields.io-Endpoint-JSON und Snippet                                             |
| `qa-doctor --cache`                   | Inkrementelle Re-Scans über einen lokalen Urteils-Cache                          |
| `qa-doctor --format mermaid`          | Testarchitektur-Diagramm für einen PR-Kommentar                                  |

`qa-doctor help <command>` gibt für jeden davon Verwendung, Beispiele und den nächsten Schritt aus.

</details>

Benötigt **Node.js ≥ 22.18** unter Windows, macOS oder Linux. Lieber global installieren? `npm i -g qa-doctor-cli`. Die Untergrenze kommt von der Build-Toolchain (tsdown zielt darauf, und die Release-Pipeline macht Smoke-Tests dagegen); die Laufzeitabhängigkeiten brauchen nicht mehr.
...
Der vollständige Katalog wird aus der Registry erzeugt, nie von Hand gepflegt: `qa-doctor rules --md`, [`docs/rules/`](docs/rules/) oder der [What-it-checks-Leitfaden](https://sergey-bar.github.io/qa-doctor/guide/what-it-checks).
...
Jede Regel wird mit einem Must-fire- **und** einem Must-not-fire-Fixture ausgeliefert, und eine Regel, die auf ihrem eigenen Negativ-Fixture anschlägt, kann nicht ausgeliefert werden. Das ist die Falsch-Positiv-Firewall; `qa-doctor doctor` erzwingt sie in der eigenen CI dieses Repositorys.

### Selector Health Score

`qa-doctor doctor:playwright` bewertet jeden Locator danach, wie er ein Element findet: so wie ein Nutzer (Rolle, Label, Text), über einen expliziten Vertrag (`data-testid`) oder über einen strukturellen Zufall (CSS-Ketten, XPath). Jede Datei bekommt einen Score von 0 bis 100:
...
**74 von 79 Regeln haben eine gegen echten OSS-Code gemessene Falsch-Positiv-Rate** (mindestens 10 von Hand klassifizierte Befunde je Regel; siehe [docs/FP-AUDIT.md](docs/FP-AUDIT.md)). Die übrigen 5 laufen auf der Schätzung des Autors und sagen das Regel für Regel in `qa-doctor explain`. `qa-doctor rules --unmeasured` listet sie auf, und jede Scan-Fußzeile meldet, wie viele der tatsächlich _ausgelösten_ Regeln gemessen sind.
...
Beförderung, Herabstufung und Reife pro Sprache: [Regel-Lebenszyklus](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle).
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `qa-doctor mcp`     | Ein [MCP](https://modelcontextprotocol.io)-Server über stdio. `scan`, `explain` und `diff` werden zu aufrufbaren Tools.                                                                    |
| `qa-doctor handoff` | Ein gespeicherter `--json`-Report wird zu einem deterministischen Markdown-Plan: was erkannt wurde, die Evidenzgrenze pro Befund, was sich **nicht** ändern darf, wie zu verifizieren ist. |
| `qa-doctor install` | Schreibt in die Agenten-Oberflächen, die dein Repo bereits hat (`.claude/`, `.cursor/`, `.kilo/`, `AGENTS.md`), damit der Agent erneut scannt, bevor er behauptet, fertig zu sein.         |
...
Die vollständige Doku-Website findest du unter <https://sergey-bar.github.io/qa-doctor/>.
...
**Version 1.** Das JSON-Schema und die Exit-Codes sind eingefrorene Verträge. TypeScript und Python haben die breiteste gemessene Abdeckung. Java und C# sind neuer; lies sie durch die [Reifetabelle](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle). Was als Nächstes kommt, ohne erfundene Termine: [die öffentliche Roadmap](https://sergey-bar.github.io/qa-doctor/reference/roadmap).
...
[Zum Leitfaden](https://sergey-bar.github.io/qa-doctor/guide/getting-started) · [Doku-Website](https://sergey-bar.github.io/qa-doctor/) · [npm](https://www.npmjs.com/package/qa-doctor-cli)

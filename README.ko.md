...
<img src="assets/readme/hero.svg" alt="QA Doctor. 테스트는 무엇이 통과했는지 알려줍니다. QA Doctor는 무엇을 믿을 수 있는지 알려줍니다." width="100%" />

<br />

QA Doctor는 실패할 수 없는 테스트와 빨간색이 될 수 없는 파이프라인을 찾아낸 뒤,<br />
...
[실제 동작 보기](#실제-동작-보기) · [빠른 시작](#빠른-시작) · [무엇을 찾는가](#qa-doctor가-찾는-것) · [점수](#신뢰도-점수) · [증거](#증거-모델) · [실행 포렌식](#런타임-포렌식) · [CI](#ci-무결성) · [에이전트](#ai-에이전트) · [보안](#신뢰와-보안) · [한계](#qa-doctor가-알려줄-수-없는-것) · [문서](#문서)
...
이 중 어느 것도 파이프라인을 빨간색으로 만들지 않고, 리뷰에서는 모두 의도된 것처럼 보입니다. 그래서 살아남습니다. QA Doctor가 실제 사례를 읽는 모습입니다.

<p align="center">
  <img src="assets/readme/scan.svg" alt="데모 저장소의 CI workflow를 한 줄씩 읽은 것입니다. QA Doctor는 각 발견 사항을 보고한 줄에 표시하고, 규칙, 무엇이 잘못되었는지, 증거 수준, 측정된 오탐률을 함께 보여줍니다." width="800" />
</p>
...
**엄격 모드.** 가장 공격적인 탐지 — `.only`, `continue-on-error`, 빈 테스트, 재시도 남용 — 는 격리 티어에 있습니다. `--strict`에서만 실행되며 `info` 심각도로 제한됩니다: 플래그를 지정하지만 절대 게이트를 닫지 않습니다. 기본 스캔(`--strict` 없는 `npx qa-doctor-cli@latest`)은 핵심 및 확장 규칙만 다룹니다. 자문 레이어도 원할 때 `--strict`를 추가하세요.

QA Doctor는 테스트 스위트와 CI workflow, 그리고 있다면 실제 실행 리포트를 읽습니다. 테스트를 실행하지 않고, 의존성을 설치하지 않으며, 스캔하는 코드를 실행하지도 않습니다. 증거가 없을 때는 확신을 지어내는 대신 그렇다고 말합니다.
...
<p align="center">
  <img src="assets/readme/how-it-works.svg" alt="QA Doctor의 동작 방식. 테스트 스위트와 CI 파이프라인을 정적으로 읽고, 실제 실행 리포트가 있으면 그것도 읽습니다. 각 발견 사항에 증거 수준과 신뢰 수준으로 가중치를 매기며, L3부터 L5까지는 실제 실행만 도달할 수 있습니다. 결과로 발견 사항, 신뢰도 점수, 그리고 고정된 종료 코드 기반의 CI 게이트를 만들어 냅니다. 에이전트 루프에서는 AI가 수정을 작성하고 QA Doctor가 다시 스캔해 그것을 증명합니다." width="880" />
</p>
...
<p align="center">
  <a href="assets/video/qa-doctor-demo.mp4">
    <img src="assets/video/qa-doctor-demo-poster.png" alt="데모 녹화의 한 프레임: 터미널 창에서 npx qa-doctor-cli@latest가 데모 저장소를 스캔하는 모습" width="900" />
  </a>
</p>

<sub>`npm run docs:video`가 실제 스캔으로부터 프레임 단위로 렌더링했으며, 화면 녹화가 아닙니다. 프레임을 선택하면 [`qa-doctor-demo.mp4`](assets/video/qa-doctor-demo.mp4)가 열립니다.</sub>

</details>

### 발견 사항 하나 자세히 보기

모든 발견 사항은 네 가지 질문에 답합니다. 어디에 있는지, QA Doctor가 얼마나 확신하는지, 그 규칙이 얼마나 자주 틀리는지, 그리고 어떻게 고치는지.
...
`qa-doctor explain QA-CI-001`은 규칙의 신뢰 기록 전체를 출력합니다. 측정된 오탐률과 그 비율로 얻은 등급도 포함됩니다.
...
`qa-doctor ci install`은 이를 GitHub Actions workflow로 작성하며, 메이저 태그 `v1`에 고정된 [action](https://github.com/Sergey-Bar/qa-doctor#readme)을 사용합니다 (`--no-action`을 쓰면 일반 `npx`). 차단해야 한다고 결정하기 전까지는 권고 모드로 유지됩니다.

| 명령                                  | 하는 일                                         |
| ------------------------------------- | ----------------------------------------------- |
| `qa-doctor`                           | Trust Report: 판정, 확신도, 다음 조치           |
| `qa-doctor --scope changed`           | 브랜치가 도입한 부분만 (CI용 형태)              |
| `qa-doctor ci install`                | 권고용 PR workflow 생성 (action 기반)           |
| `qa-doctor explain QA-CI-001`         | 무엇이, 왜, 어떻게 고치는지와 측정된 FP 비율    |
| `qa-doctor why src/a.spec.ts:42`      | 바로 이 줄이 표시된 이유. 차단하지 않습니다.    |
| `qa-doctor forensics ./test-results/` | 실제 실행에서 얻은 런타임 증거                  |
| `qa-doctor trust-report`              | 독립형 Trust Artifact (md + json)               |
| `qa-doctor handoff`                   | 코딩 에이전트를 위한 수정 계획                  |
| `qa-doctor --json` / `--format sarif` | 기계가 읽을 수 있는 출력, GitHub Code Scanning  |
| `qa-doctor --format codequality`      | GitLab Code Quality 리포트 (MR 위젯 아티팩트)   |
| `qa-doctor --strict`                  | quarantine 등급 규칙도 실행 (FP 위험이 더 높음) |
...
| 명령                                  | 하는 일                                                         |
| ------------------------------------- | --------------------------------------------------------------- |
| `qa-doctor --classic`                 | Trust Report 이전의 점수 배너 화면                              |
| `qa-doctor explain verdict`           | 저장된 스캔의 판정이 왜 그렇게 나왔는지                         |
| `qa-doctor triage ./test-results/`    | 안내형 분류. 모든 행이 다음 조치로 끝납니다.                    |
| `qa-doctor pw-report ./test-results/` | Playwright 실행 요약: 재시도, 불안정한 테스트, 가장 느린 테스트 |
| `qa-doctor doctor:playwright`         | Playwright 전용 심층 스캔과 Selector Health Score               |
| `qa-doctor fix --dry-run` / `fix`     | 안전한 자동 수정. 각 수정은 재스캔으로 적용되었음을 증명합니다  |
| `qa-doctor baseline` / `diff`         | 발견 사항의 스냅숏을 만든 뒤 새롭거나 악화된 것만 보고          |
| `qa-doctor impact --since <ref>`      | 커밋이 도입하고 해결한 것                                       |
| `qa-doctor summary`                   | 리포트로부터 CI 주석과 step 요약 생성                           |
| `qa-doctor pr-comment`                | 범위를 한정한 PR 댓글 (Markdown)                                |
| `qa-doctor debt`                      | 비용 모델을 포함한 테스트 부채 목록                             |
| `qa-doctor handover`                  | 새 QA 엔지니어를 위한 스위트 온보딩 지도                        |
| `qa-doctor init`                      | 프레임워크를 감지하고 설정 체크리스트 출력                      |
| `qa-doctor suppressions`              | 억제된 발견 사항 목록 (거버넌스용)                              |
| `qa-doctor rules --unmeasured`        | 측정이 아니라 가정으로 동작하는 규칙                            |
| `qa-doctor rules --md`                | 전체 규칙 카탈로그 (JSON 또는 Markdown)                         |
| `qa-doctor doctor`                    | QA Doctor 자체 규칙 기반에 대한 자체 감사                       |
| `qa-doctor create-rule <ID>`          | 새 규칙과 그 fixture의 뼈대 생성                                |
| `qa-doctor stats`                     | 지금까지 본 수정의 로컬 누적 카운터                             |
| `qa-doctor badge`                     | shields.io 엔드포인트 JSON과 스니펫                             |
| `qa-doctor --cache`                   | 로컬 판정 캐시를 이용한 증분 재스캔                             |
| `qa-doctor --format mermaid`          | PR 댓글용 테스트 아키텍처 다이어그램                            |

`qa-doctor help <command>`는 어떤 명령이든 사용법, 예시, 다음 단계를 출력합니다.

</details>

Windows, macOS 또는 Linux에서 **Node.js ≥ 22.18**이 필요합니다. 전역 설치를 원하시나요? `npm i -g qa-doctor-cli`. 이 최소 버전은 빌드 도구 체인에서 옵니다 (tsdown이 이를 대상으로 하고 릴리스 파이프라인이 이에 대해 스모크 테스트를 합니다). 런타임 의존성은 그 이상을 요구하지 않습니다.
...
전체 카탈로그는 레지스트리에서 생성되며 손으로 관리하지 않습니다: `qa-doctor rules --md`, [`docs/rules/`](docs/rules/), 또는 [검사 항목 가이드](https://sergey-bar.github.io/qa-doctor/guide/what-it-checks).
...
모든 규칙은 must-fire **그리고** must-not-fire fixture와 함께 배포되며, 자신의 음성 fixture에서 발동하는 규칙은 배포될 수 없습니다. 이것이 오탐 방화벽입니다. `qa-doctor doctor`가 이 저장소의 CI에서 이를 강제합니다.

### Selector Health Score

`qa-doctor doctor:playwright`는 각 locator가 요소를 찾는 방식으로 등급을 매깁니다. 사용자처럼 찾는지 (역할, 레이블, 텍스트), 명시적 계약을 통하는지 (`data-testid`), 아니면 구조적 우연에 기대는지 (CSS 체인, XPath). 파일마다 0에서 100 사이의 점수를 받습니다.
...
**100점이 의미하지 않는 것.** 소프트웨어가 올바르다는 뜻도, 스위트가 충분하다는 뜻도, 제품에 결함이 없다는 뜻도 아닙니다. 의미하는 것은 하나뿐입니다. **이 스캔과 이 증거 모델에서 QA Doctor가 평가한 규칙 중 어느 것도 감점을 만들지 않았다는 것.**

<br />

## 증거 모델

모든 발견 사항에는 두 개의 라벨이 붙습니다. QA Doctor가 얼마나 확신하는지, 그리고 그 발견 사항이 어디까지 확인되었는지. 이것이 패턴을 보고하는 도구와 릴리스 게이트로 삼을 수 있는 도구의 차이입니다.
...
**어디까지 확인했는가 — 신뢰 수준.** 대부분의 발견 사항은 코드를 읽어서 나옵니다. 실제 테스트 실행 리포트를 QA Doctor에 주면 코드가 실제로 실행되었는지 확인할 수 있습니다.
...
**79개 규칙 중 74개는 실제 OSS 코드로 측정한 오탐률을 갖고 있습니다** (규칙마다 손으로 분류한 발견 사항 10개 이상. [docs/FP-AUDIT.md](docs/FP-AUDIT.md) 참고). 나머지 5개는 작성자의 추정치로 배포되며, `qa-doctor explain`에서 규칙별로 그 사실을 밝힙니다. `qa-doctor rules --unmeasured`가 이를 나열하고, 모든 스캔의 하단에는 실제로 _발동한_ 규칙 중 몇 개가 측정되었는지 보고합니다.
...
승격, 강등, 언어별 성숙도: [규칙 생명 주기](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle).

### 왜 이것은 린터가 아닌가

린터는 코드가 규칙을 따르는지 알려줍니다. QA Doctor는 검증을 믿을 수 있는지 알려줍니다.

|                                                       | 린터 (ESLint, SonarQube) | 커버리지 도구 | AI 코드 리뷰 | **QA Doctor** |
| ----------------------------------------------------- | :----------------------: | :-----------: | :----------: | :-----------: |
...
AI 리뷰도 함께 쓰세요. AI 리뷰는 어떤 패턴으로도 찾을 수 없는 뉘앙스, 의도, 설계 결함을 잡아냅니다. QA Doctor는 AI 리뷰가 의도된 것처럼 보여서 놓치는 것을 잡습니다: 커밋된 `.only`, 삼켜진 종료 코드, 테스트 job에 붙은 `continue-on-error`. 이런 것에는 추론이 아니라 스캔이 필요합니다.
...
`TRUE-FLAKE`는 테스트가 재시도되었다는 뜻이 아닙니다. 테스트가 **적어도 한 번의 시도에서 실패한 뒤 초록색으로 끝났다**는 뜻입니다. 운 좋은 통과이며, 최종 체크가 무엇을 말하든 표시됩니다. `qa-doctor triage`는 그 이력을 격리 제안으로 바꾸고, `qa-doctor pw-report`는 실행을 요약합니다. 발견 사항을 신뢰 수준 L3 이상으로 올리는 것도 바로 이 실행 리포트입니다.

<br />

## CI 무결성

테스트는 통과하는데 그것을 둘러싼 파이프라인은 실패할 수 없는 경우가 있습니다. QA Doctor는 workflow도 읽습니다: `continue-on-error`, `|| true`, 전달되지 않는 종료 코드, 항상 성공하는 step, 사용되지만 생성되지 않는 리포트, 그리고 차단해야 할 이벤트에서 건너뛰는 게이트. 각 발견 사항은 job, step, 줄을 지목하며 자체 증거 수준을 가집니다.
...
**AI가 수정을 작성하고, QA Doctor가 그것을 검증합니다.** 증명은 재스캔에서 나오며, 에이전트 스스로의 성공 보고에서 나오지 않습니다.

| 명령                | 에이전트가 받는 것                                                                                                                                      |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa-doctor mcp`     | stdio 기반 [MCP](https://modelcontextprotocol.io) 서버. `scan`, `explain`, `diff`가 호출 가능한 도구가 됩니다.                                          |
| `qa-doctor handoff` | 저장된 `--json` 리포트가 결정론적인 Markdown 계획이 됩니다: 무엇이 탐지되었는지, 발견 사항별 증거 경계, 바뀌면 **안 되는** 것, 검증 방법.               |
| `qa-doctor install` | 저장소에 이미 있는 에이전트 설정 위치 (`.claude/`, `.cursor/`, `.kilo/`, `AGENTS.md`)에 작성해, 에이전트가 완료했다고 말하기 전에 다시 스캔하게 합니다. |
...
## QA Doctor가 알려줄 수 없는 것

- **테스트를 실행하지 않습니다.** 깨끗한 스캔이 통과하는 스위트를 뜻하지는 않습니다.
- **단언이 _틀렸다_ 는 것은 알려줄 수 없습니다.** `expect(total).toBe(41)`은 건강해 보입니다. QA Doctor가 찾는 것은 _실패할 수 없는_ 테스트와 _빨간색이 될 수 없는_ 파이프라인이지, 엉뚱한 것을 확인하는 테스트가 아닙니다.
...
전체 문서 사이트는 <https://sergey-bar.github.io/qa-doctor/>에 있습니다.
...
**버전 1.** JSON 스키마와 종료 코드는 고정된 계약입니다. TypeScript와 Python이 가장 넓은 측정 범위를 갖습니다. Java와 C#은 비교적 새로우니 [성숙도 표](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle)와 함께 보세요. 앞으로의 계획 (지어낸 날짜 없음): [공개 로드맵](https://sergey-bar.github.io/qa-doctor/reference/roadmap).
...
[가이드 읽기](https://sergey-bar.github.io/qa-doctor/guide/getting-started) · [문서 사이트](https://sergey-bar.github.io/qa-doctor/) · [npm](https://www.npmjs.com/package/qa-doctor-cli)

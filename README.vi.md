...
[Xem cách hoạt động](#xem-cách-hoạt-động) · [Bắt đầu nhanh](#bắt-đầu-nhanh) · [Phát hiện gì](#qa-doctor-phát-hiện-gì) · [Điểm](#điểm-đáng-tin) · [Bằng chứng](#mô-hình-bằng-chứng) · [Phân tích lần chạy](#phân-tích-pháp-chứng-lúc-chạy) · [CI](#tính-toàn-vẹn-ci) · [Tác tử](#tác-tử-ai) · [Bảo mật](#tin-cậy-và-bảo-mật) · [Giới hạn](#những-điều-qa-doctor-không-thể-cho-bạn-biết) · [Tài liệu](#tài-liệu)
...
<p align="center">
  <a href="assets/video/qa-doctor-demo.mp4">
    <img src="assets/video/qa-doctor-demo-poster.png" alt="Một khung hình từ bản ghi demo: npx qa-doctor-cli@latest đang quét kho demo trong cửa sổ terminal" width="900" />
  </a>
</p>

<sub>Được dựng từng khung hình từ một lần quét thật bằng `npm run docs:video`; không bao giờ quay màn hình. Chọn khung hình để mở [`qa-doctor-demo.mp4`](assets/video/qa-doctor-demo.mp4).</sub>
...
`qa-doctor explain QA-CI-001` in ra toàn bộ hồ sơ tin cậy của một quy tắc, gồm cả tỷ lệ dương tính giả đã đo và cấp mà tỷ lệ đó mang lại cho nó:
...
`qa-doctor ci install` ghi điều đó thành một workflow GitHub Actions, dùng [action](https://github.com/Sergey-Bar/qa-doctor#readme) được ghim vào tag chính `v1` (hoặc `npx` thuần với `--no-action`). Nó chỉ mang tính tư vấn cho đến khi bạn quyết định nó nên chặn.

| Lệnh                                  | Chức năng                                              |
| ------------------------------------- | ------------------------------------------------------ |
| `qa-doctor`                           | Trust Report: kết luận, độ tin, hành động tiếp theo    |
| `qa-doctor --scope changed`           | Chỉ những gì nhánh của bạn đưa vào (dạng dùng cho CI)  |
| `qa-doctor ci install`                | Tạo workflow PR mang tính tư vấn (dựa trên action)     |
| `qa-doctor explain QA-CI-001`         | Cái gì, vì sao và cách sửa, kèm tỷ lệ FP đã đo         |
| `qa-doctor why src/a.spec.ts:42`      | Vì sao chính dòng này bị đánh dấu. Không bao giờ chặn. |
| `qa-doctor forensics ./test-results/` | Bằng chứng runtime từ một lần chạy thật                |
| `qa-doctor trust-report`              | Trust Artifact độc lập (md + json)                     |
| `qa-doctor handoff`                   | Kế hoạch khắc phục cho tác tử lập trình                |
| `qa-doctor --json` / `--format sarif` | Đầu ra máy đọc được, GitHub Code Scanning              |
| `qa-doctor --format codequality`      | Báo cáo GitLab Code Quality (artifact cho widget MR)   |
| `qa-doctor --strict`                  | Chạy cả các quy tắc cấp quarantine (rủi ro FP cao hơn) |
...
| Lệnh                                  | Chức năng                                                                   |
| ------------------------------------- | --------------------------------------------------------------------------- |
| `qa-doctor --classic`                 | Giao diện banner điểm từ trước khi có Trust Report                          |
| `qa-doctor explain verdict`           | Vì sao kết luận của lần quét đã lưu lại như vậy                             |
| `qa-doctor triage ./test-results/`    | Phân loại có hướng dẫn. Mỗi hàng kết thúc bằng một hành động tiếp theo.     |
| `qa-doctor pw-report ./test-results/` | Tóm tắt lần chạy Playwright: số lần thử lại, test chập chờn, test chậm nhất |
| `qa-doctor doctor:playwright`         | Quét sâu chỉ dành cho Playwright kèm Selector Health Score                  |
| `qa-doctor fix --dry-run` / `fix`     | Tự động sửa an toàn, mỗi bản sửa được quét lại để chứng minh nó có hiệu lực |
| `qa-doctor baseline` / `diff`         | Chụp nhanh các phát hiện, sau đó chỉ báo cáo cái mới hoặc tệ hơn            |
| `qa-doctor impact --since <ref>`      | Một commit đã đưa vào và giải quyết những gì                                |
| `qa-doctor summary`                   | Chú thích CI và tóm tắt step từ một báo cáo                                 |
| `qa-doctor pr-comment`                | Một bình luận PR có phạm vi, dạng Markdown                                  |
| `qa-doctor debt`                      | Sổ nợ test kèm mô hình chi phí                                              |
| `qa-doctor handover`                  | Bản đồ làm quen bộ test cho kỹ sư QA mới                                    |
| `qa-doctor init`                      | Phát hiện framework, in danh sách kiểm tra thiết lập                        |
| `qa-doctor suppressions`              | Liệt kê các phát hiện bị chặn, phục vụ quản trị                             |
| `qa-doctor rules --unmeasured`        | Những quy tắc chạy dựa trên giả định, không phải đo lường                   |
| `qa-doctor rules --md`                | Danh mục quy tắc đầy đủ (JSON hoặc Markdown)                                |
| `qa-doctor doctor`                    | Tự kiểm toán cơ sở quy tắc của chính QA Doctor                              |
| `qa-doctor create-rule <ID>`          | Tạo khung cho một quy tắc mới và các fixture của nó                         |
| `qa-doctor stats`                     | Bộ đếm cục bộ mọi bản sửa từng thấy                                         |
| `qa-doctor badge`                     | JSON cho endpoint shields.io và đoạn mã                                     |
| `qa-doctor --cache`                   | Quét lại tăng dần qua bộ đệm kết luận cục bộ                                |
| `qa-doctor --format mermaid`          | Sơ đồ kiến trúc test cho một bình luận PR                                   |

`qa-doctor help <command>` in cách dùng, ví dụ và bước tiếp theo cho bất kỳ lệnh nào.

</details>

Yêu cầu **Node.js ≥ 22.18** trên Windows, macOS hoặc Linux. Muốn cài toàn cục? `npm i -g qa-doctor-cli`. Mức tối thiểu này đến từ chuỗi công cụ build (tsdown nhắm tới nó và pipeline phát hành chạy smoke test trên nó); các dependency lúc chạy không cần gì hơn.
...
Danh mục đầy đủ được tạo từ sổ đăng ký, không bao giờ duy trì thủ công: `qa-doctor rules --md`, [`docs/rules/`](docs/rules/), hoặc [hướng dẫn về những gì nó kiểm tra](https://sergey-bar.github.io/qa-doctor/guide/what-it-checks).
...
Mỗi quy tắc được phát hành cùng một fixture must-fire **và** một fixture must-not-fire, và quy tắc nào kích hoạt trên chính fixture âm của nó thì không thể phát hành. Đó là tường lửa chống dương tính giả; `qa-doctor doctor` thực thi nó trong CI của chính kho này.

### Selector Health Score

`qa-doctor doctor:playwright` chấm điểm mỗi locator theo cách nó tìm phần tử: theo cách người dùng tìm (vai trò, nhãn, văn bản), qua một hợp đồng rõ ràng (`data-testid`), hay nhờ một sự tình cờ về cấu trúc (chuỗi CSS, XPath). Mỗi tệp nhận điểm từ 0 đến 100:
...
Nâng cấp, hạ cấp và độ trưởng thành theo ngôn ngữ: [vòng đời quy tắc](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle).
...
`TRUE-FLAKE` không có nghĩa là test đã được thử lại. Nó có nghĩa là test **đã thất bại ít nhất một lần thử rồi kết thúc với màu xanh**: một lần qua may mắn, bị đánh dấu bất kể dấu tích cuối cùng nói gì. `qa-doctor triage` biến lịch sử đó thành một đề xuất cách ly, và `qa-doctor pw-report` tóm tắt một lần chạy. Chính những báo cáo chạy này là thứ nâng phát hiện lên mức tin cậy L3 trở lên.
...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa-doctor mcp`     | Một máy chủ [MCP](https://modelcontextprotocol.io) qua stdio. `scan`, `explain` và `diff` trở thành công cụ có thể gọi.                                                             |
| `qa-doctor handoff` | Một báo cáo `--json` đã lưu trở thành một kế hoạch Markdown tất định: đã phát hiện gì, ranh giới bằng chứng của từng phát hiện, những gì **không** được thay đổi, và cách xác minh. |
| `qa-doctor install` | Ghi vào các vị trí dành cho tác tử mà kho của bạn đã có (`.claude/`, `.cursor/`, `.kilo/`, `AGENTS.md`) để tác tử quét lại trước khi khẳng định là đã xong.                         |
...
**Plugin không chạy trong sandbox.** Plugin JS (`qa-doctor-rules/*.mjs`, hoặc các gói npm liệt kê dưới `"plugins"`) chạy với toàn quyền của Node, cùng mô hình tin cậy như plugin của ESLint hay Vitest. Việc tải chúng phải được bật **cho từng lần quét**: không có `--enable-plugins` (hoặc `QA_DOCTOR_ENABLE_PLUGINS=1`) thì mã nguồn của chúng không bao giờ được tải, và một thông báo trên stderr liệt kê những gì đã bị bỏ qua. Manifest quy tắc dạng JSON không thực thi mã, và các tiền tố ID của quy tắc core được giữ riêng để plugin không thể mạo danh chúng. Báo cáo lỗ hổng qua [SECURITY.md](SECURITY.md).

**Nó tự chạy trên chính mình.** Một công cụ tin cậy xác minh chẳng có chỗ đứng nếu chính nó không thể được xác minh. Mỗi lần chạy CI đều quét kho này bằng bản build mà chính lần chạy đó tạo ra. Cổng thất bại với bất kỳ phát hiện nào ở mức error, và cả khi lần quét là **một phần** hoặc có **quy tắc bị sập**, vì một lần tự quét bị cắt cụt mà không báo cáo gì chính là màu xanh giả mà dự án này tồn tại để bắt. `qa-doctor doctor` kiểm toán lại cơ sở quy tắc trong cùng lần chạy (tường lửa fixture, tính trung thực của các cấp, trần của cấp core), và một kiểm tra có kết quả INCONCLUSIVE sẽ thất bại y như một kiểm tra thất bại. Cả hai báo cáo được tải lên dưới dạng artifact của build.
...
Trang tài liệu đầy đủ ở <https://sergey-bar.github.io/qa-doctor/>.
...
**Phiên bản 1.** Schema JSON và mã thoát là những hợp đồng đã đóng băng. TypeScript và Python có độ phủ đã đo rộng nhất. Java và C# mới hơn; hãy đọc chúng qua [bảng độ trưởng thành](https://sergey-bar.github.io/qa-doctor/reference/rule-lifecycle). Những gì sắp tới, không có ngày tháng bịa đặt: [lộ trình công khai](https://sergey-bar.github.io/qa-doctor/reference/roadmap).
...
[Đọc hướng dẫn](https://sergey-bar.github.io/qa-doctor/guide/getting-started) · [Trang tài liệu](https://sergey-bar.github.io/qa-doctor/) · [npm](https://www.npmjs.com/package/qa-doctor-cli)

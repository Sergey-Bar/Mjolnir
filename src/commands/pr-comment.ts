...
export function renderPrComment(
  ...
  if (usingDiff) {
    ...
    lines.push(
      "No baseline was found — showing the full scan. Run `qa-doctor baseline` on the base branch to scope future comments to just what changed.",
    );
  }
  lines.push("");
  ...
  if (findings.length === 0) {
    ...
    if (overflow > 0) {
      lines.push(
        `_...and ${overflow} more overall — run \`qa-doctor\` locally (or \`--verbose\`) for the full list._`,
      );
    }
  }
  ...
  lines.push("");
  lines.push(
    `_Advisory only — this comment never blocks merging. Generated by [QA Doctor](${options.repoUrl ?? "https://github.com/Sergey-Bar/qa-doctor"})._`,
  );

 ...
 * QA Doctor's own rule metadata — the generator produces INSTRUCTIONS,
...
export function renderHandoff(
  ...
  const lines: string[] = [];
  lines.push("### 🔨 QA Doctor — Fix Handoff");
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `_Generated by [QA Doctor](https://github.com/Sergey-Bar/qa-doctor) — evidence and verification; the agent remains responsible for every change._`,
  );
  return lines.join("\n");
}
...
export function runHandoffCommand(
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--category" || a === "--rules") {
      ...
      if (a === "--category" && !isValidCategory(val)) {
        io.err(`qa-doctor handoff: unknown --category: ${val}`);
  ...
  if (!reportExists(reportPath)) {
    io.err(`qa-doctor handoff: report file not found: ${reportPath}`);
    io.err(
  ...
  try {
    ...
    io.err(`qa-doctor handoff: cannot read ${reportPath}: ${errorText(err)}`);
    return 2;
  }

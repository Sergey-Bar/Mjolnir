/**
 * QA-JV-108 — Playwright-Java: hardcoded environment URLs.
 * Severity: warning · Confidence: high · deterministic-defect
 * Absolute URLs in specs hit production by accident and break between
 * environments — same meaning as QA-PW-123/QA-PY-108.
 *
 * IMPORTANT (verified in docs/JAVA-CSHARP-IDIOM-MAPPING.md against the
 * official Playwright Java docs): navigation in Java is
 * `page.navigate(url)`, NOT `page.goto(url)` like every other
 * Playwright language binding. A regex copied from the JS/Python rule
 * would silently never match a single real Java test file.
 */

import { defineRule } from "../rule.js";
import type { Finding } from "../../types.js";

export const jvHardcodedUrl = defineRule({
  id: "QA-JV-108",
  category: "QA-PW",

    ...
    for (const [jobName, job] of Object.entries(doc.jobs)) {
      for (const step of job?.steps ?? []) {
        ...
        if (step?.uses && /retry/i.test(step.uses)) {
          ...
          const command = String(withCfg['command'] ?? withCfg['max_tries'] ?? '');
          const runsTests = /\b(?:npm|yarn|pnpm)\s+(?:test|run\s+test)|\b(?:jest|vitest|pytest|playwright)\b/.test(
            command,
          );
        ...
        if (step?.run && /\bfor\b[^]*retry|max_attempts|until.*succeed/i.test(step.run) && /test/i.test(step.run)) {
          findings.push({
            severity: 'warning',
            confidence: 'medium',

...
export function computeDimensions(findings: Finding[]): DimensionScore[] {
  ...
  for (const f of findings) {
    ...
    if (!dim) {
      dim = { category: f.category, score: 100, errors: 0, warnings: 0, infos: 0 };
  ...
  for (const dim of byCategory.values()) {
    dim.score = Math.max(
      0,
      100 - dim.errors * DEDUCTIONS.error - dim.warnings * DEDUCTIONS.warning - dim.infos * DEDUCTIONS.info,
    );
  }
  return [...byCategory.values()].sort((a, b) => a.category.localeCompare(b.category));
}

export function computeTotal(dimensions: DimensionScore[], findings: Finding[]): number {

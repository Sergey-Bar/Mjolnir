...
// Audit C3: the default sinks must stay variadic — `io.err("qa-doctor
...
export function internalErrorMessage(
  ...
  emit("qa-doctor internal error — this is a bug in QA Doctor, not your repo:");
  emit(`  ${message}`);
  if (debug && err instanceof Error && err.stack) {
    emit(err.stack);
  }
  emit("Rerun with --debug for the stack trace. Please report this:");
  emit("  https://github.com/Sergey-Bar/qa-doctor/issues");
}

...
export const typescriptAdapter: LanguageAdapter = {
  ...
  discoverTestFiles(ctx: ScanContext): void {
    walk(ctx.workspace.root, ctx.workspace.root, ctx.testFiles, ctx.deadline, ctx.onSkippedFile);
  },
...
function walk(dir: string, root: string, out: string[], deadline: number, onSkipped: () => void): void {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (rel.split('/').length <= LIMITS.maxDepth) walk(full, root, out, deadline, onSkipped);

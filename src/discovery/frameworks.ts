...
/** Reads testMatch-ish globs from a jest config when trivially parseable. */
export function readJestTestDir(ws: Workspace): string | null {
  for (const f of CONFIG_FILES.jest) {
    const p = join(ws.root, f);
    if (!existsSync(p)) continue;
    try {
      const text = readFileSync(p, 'utf8');
      const m = /rootDir\s*:\s*['"`]([^'"`]+)['"`]/.exec(text);
      return m?.[1] ?? null;
    } catch {
      return null;
    }

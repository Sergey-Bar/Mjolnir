...
export function loadSuppressions(root: string): SuppressionReport {
  ...
  try {
    const cfg = JSON.parse(readFileSync(configPath, 'utf8')) as { ignore?: IgnoreEntry[] };
    const entries = (cfg.ignore ?? []).map((ign) => ({
      ...ign,
      status: isSuppressionActive(ign) ? ('active' as const) : ('expired' as const),
    }));
    return {
...
export function renderSuppressions(report: SuppressionReport): string {
  ...
  for (const e of report.entries) {
    lines.push(`${e.status === 'active' ? '●' : '○'} ${e.ruleId} — ${e.reason}${e.expires ? ` (expires ${e.expires})` : ''}`);
  }
  lines.push('');
  return lines.join('\n');
}

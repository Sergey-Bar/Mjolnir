/* eslint-disable security/detect-non-literal-fs-filename -- external rule loading reads user-declared paths */
/* eslint-disable security/detect-non-literal-regexp -- external rule patterns are user-declared regex sources */
/**
...
function loadJsonRule(path: string, result: LoadedExternalRules): void {
  ...
  for (const p of patterns) {
    ...
    const quantifiers = (p as string).match(/[+*?]|\{\d/g);
    if (quantifiers && quantifiers.length > 64) {
      result.errors.push(
        `external rule ${id} has a pattern with more than 64 quantifier/wildcard tokens — likely catastrophic backtracking.`,
      );
      return;
    }
  }
...
async function loadModuleRules(
  ...
  for (const rule of rules) {
    ...
    if (typeof r.severity === "string" && !ALLOWED_SEVERITIES.has(r.severity)) {
      result.errors.push(
        `external rule ${r.id} declares an invalid "severity" (need error|warning|info).`,
      );
      continue;
    }
    if (typeof r.category === "string" && !ALLOWED_CATEGORIES.has(r.category)) {
      result.errors.push(
        `external rule ${r.id} declares an invalid "category" (need ${[...ALLOWED_CATEGORIES].join("|")}).`,
      );
      continue;
    }
    if (
      ...
      result.errors.push(
        `external rule ${r.id} declares an invalid "appliesTo" (need ${[...ALLOWED_APPLIES_TO].join("|")}).`,
      );
      continue;
    }
    if (
      typeof (r as { qaImpact?: unknown }).qaImpact === "string" &&
      !ALLOWED_QA_IMPACTS.has((r as { qaImpact?: string }).qaImpact as string)
    ) {
      result.errors.push(
        `external rule ${r.id} declares an invalid "qaImpact" (need ${[...ALLOWED_QA_IMPACTS].join("|")}).`,
      );
      continue;
    }

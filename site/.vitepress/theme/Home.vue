...
// so it can never drift from what the tool actually prints.
import scanHero from "../../../assets/readme/terminal-hero.svg";
...
<template>
  <main class="mj">
    ...
    <section class="showcase">
      <div class="show-grid">
        <div class="show-copy" data-reveal>
          ...
          <p class="fine">
            ...
            rendered to SVG, nothing trimmed.
          </p>
        </div>
      ...
      <figure class="scan" data-reveal>
        <div class="scan-bar" aria-hidden="true">
          <span /><span /><span />
          <code>mjolnir ./examples/demo-repo --verbose</code>
        </div>
        <div class="scan-scroll">
          <img
            :src="scanHero"
            alt="Mjölnir's terminal report on a demo repo: WORTHINESS 70/100 NEEDS WORK, a diagnostics-by-category breakdown, and every finding with its rule ID and line number across CI, Playwright and test-hygiene rules."
            loading="lazy"
          />
        </div>
      </figure>
...
.scan img {
  display: block;
  width: 100%;
  /* keep the report text legible on narrow screens — the wrapper scrolls */
  min-width: 560px;
  height: auto;
}

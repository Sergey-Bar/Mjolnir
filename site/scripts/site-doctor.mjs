 ...
 *
 * That check now lives in `scripts/brand-doctor.mjs` rule 5, which
 * checks the same tables plus the verdict rows this one never parsed —
 * where a phantom light ramp and two stale dark values had been sitting
 * unnoticed. There is one implementation, not two: a second copy would
 * be a second thing to keep in sync, which is the very defect the brand
 * gate exists to prevent.
 * ------------------------------------------------------------------ */

import { rule5 } from "../../scripts/brand-doctor.mjs";
...
function checkBrandPalette() {
  ...
  return {
    n: 8,
    name: "Brand palette",
    detail: `${r.detail} (delegated to brand-doctor rule 5)`,
    ...(r.gap ? { gap: r.gap } : { failures: r.failures }),
  };
}

...
<!-- mjolnir:managed v1.0.8 -->

Run `/npx mjolnir-qa@1.0.8 . --scope changed` before finishing any task that touched test code.
...
- npx mjolnir-qa@1.0.8 why <file>:<line> explains any finding (evidence level, measured FP rate, fix).
- npx mjolnir-qa@1.0.8 handoff renders a full remediation plan from a saved --json report.
- Agent loop: establish the before-state once with npx mjolnir-qa@1.0.8 baseline (commits .mjolnir/baseline.json); after fixing, npx mjolnir-qa@1.0.8 verify prints the before/after digest — resolved (per §15 lifecycle) / new / unchanged by ruleId+location, and the score delta. Exit 0 clean, 1 new errors, 2 partial or no baseline.

...
<!-- mjolnir:managed v1.0.5 -->

Run `/npx mjolnir-qa@1.0.5 . --scope changed` before finishing any task that touched test code.
...
- npx mjolnir-qa@1.0.5 why <file>:<line> explains any finding (evidence level, measured FP rate, fix).
- npx mjolnir-qa@1.0.5 handoff renders a full remediation plan from a saved --json report.
- Agent loop: establish the before-state once with npx mjolnir-qa@1.0.5 baseline (commits .mjolnir/baseline.json); after fixing, npx mjolnir-qa@1.0.5 verify prints the before/after digest — resolved (per §15 lifecycle) / new / unchanged by ruleId+location, and the score delta. Exit 0 clean, 1 new errors, 2 partial or no baseline.

...
<!-- mjolnir:managed v1.1.0 -->

Run `/npx mjolnir-qa@1.1.0 . --scope changed` before finishing any task that touched test code.
...
- npx mjolnir-qa@1.1.0 why <file>:<line> explains any finding (evidence level, measured FP rate, fix).
- npx mjolnir-qa@1.1.0 handoff renders a full remediation plan from a saved --json report.
- Agent loop: establish the before-state once with npx mjolnir-qa@1.1.0 baseline (commits .mjolnir/baseline.json); after fixing, npx mjolnir-qa@1.1.0 verify prints the before/after digest — resolved (per §15 lifecycle) / new / unchanged by ruleId+location, and the score delta. Exit 0 clean, 1 new errors, 2 partial or no baseline.

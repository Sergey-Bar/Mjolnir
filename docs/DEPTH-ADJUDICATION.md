...
### `lexical-artifact` — the defect IS the lexical artifact (text = finding) (9 rules)

- **QA-CYP-001** (20% FP at n=15): cy.wait with a numeric-literal argument only — alias waits (cy.wait('@…')) are the legitimate routed-request form and never fire
- **QA-CYP-002** (0% FP at n=10): Cypress .only() is a lexical modifier — the regex matches the literal `.only(` call target in source text
- **QA-PW-146** (12% FP at n=17): CSS/XPath string selectors in locator()/waitForSelector() calls and raw page.$/page.$$ handles detected via regex on the raw text view
- **QA-PW-147** (unmeasured): codegen recorder's default test title ('test', 'test 1', …) committed verbatim — the regex matches the literal title string on the RAW text view
...
- **QA-SE-001** (0% FP at n=10): sleep-then-lookup sequence: Thread.sleep/Thread.Sleep/time.sleep regex match followed by findElement/FindElement/find_element within 3 lines on code-only view
- **QA-SE-002** (8% FP at n=13): sleep-then-lookup sequence: Thread.sleep/Thread.Sleep/time.sleep regex match followed by findElement/FindElement/find_element within 3 lines on code-only view
- **QA-SE-003** (0% FP at n=11): sleep-then-lookup sequence: Thread.sleep/Thread.Sleep/time.sleep regex match followed by findElement/FindElement/find_element within 3 lines on code-only view
...
- **QA-CYP-003** (0% FP at n=10): chromeWebSecurity:false in cypress.config is a string-content match in the config file — the regex detects the boolean literal

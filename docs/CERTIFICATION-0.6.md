...
| Trust Report (WI-5)               | ✅             | `src/reporter/trust-report.ts`; default `qa-doctor` output; five-question acceptance test; `--classic` identity test                                                                                                                                      |
...
| Published reporter (WI-10)        | ⚠️ **partial** | package ready (`packages/playwright-reporter`), ingestion contract locked (`qa-doctor.report.json`), version-sync gate ACTIVE in advisory mode (`scripts/check-reporter-version.ts`); the actual npm publish happens through the owner-gated release flow |
| Trust Artifact basics (WI-6)      | ✅             | `qa-doctor trust-report` → deterministic self-contained `qa-doctor-trust-report.{md,json}`; byte-identical reproducibility test                                                                                                                           |

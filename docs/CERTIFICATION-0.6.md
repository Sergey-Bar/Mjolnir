...
| Published reporter (WI-10)        | ⚠️ **partial** | package ready (`packages/playwright-reporter`), ingestion contract locked (`mjolnir.report.json`), version-sync gate ACTIVE in advisory mode (`scripts/check-reporter-version.ts`); status is post-MVP unpublished/source-only, and actual npm publish happens through a later owner-gated release flow |
...
1. **6 rules PROVISIONAL** (down from 21 at the 0.6.7 cut — the WI-14
   ...
   effective core.
2. **Reporter post-MVP unpublished/source-only** — npm installation is
   not part of the MVP path; publish is a later owner's release action,
   and the gate ramps to blocking after 3 green release cycles (§14).

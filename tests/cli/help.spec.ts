...
describe("root help", () => {
  ...
  it("keeps long subcommand usage strings separated from descriptions", () => {
    const text = renderRootHelp();
  ...
  it("wraps long help rows cleanly at narrow terminal widths", () => {
    ...
    expect(text).toContain("    generate the PR workflow");
    expect(text).not.toMatch(/\[--force\]generate/);
  });

...
## 16. pytest-dev-pytest — testing/test_mark.py:1036

**Message:** Skipped test detected: `non-strict xfail`.

```
    1031|         )
    1032|         reprec = pytester.inline_run("-k", "mykeyword", p)
    1033|         _passed, _skipped, failed = reprec.countoutcomes()
    1034|         assert failed == 1
    1035|
>>> 1036|     @pytest.mark.xfail
    1037|     def test_keyword_extra_dash(self, pytester: Pytester) -> None:
    1038|         p = pytester.makepyfile(
    1039|             """
    1040|            def test_one():
    1041|                assert 0
```
...
## 17. pytest-dev-pytest — testing/test_runner.py:604

**Message:** Skipped test detected: `non-strict xfail`.

```
     599|
     600| # design question: do we want general hooks in python files?
     601| # then something like the following functional tests makes sense
     602|
     603|
>>>  604| @pytest.mark.xfail
     605| def test_runtest_in_module_ordering(pytester: Pytester) -> None:
     606|     p1 = pytester.makepyfile(
     607|         """
     608|         import pytest
     609|         def pytest_runtest_setup(item): # runs after class-level!
```

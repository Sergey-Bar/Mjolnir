...
const HOSTILE_SNIPPETS: Array<[string, string]> = [
  ...
  ['prototype-pollution', `const o = JSON.parse('{"__proto__":{"polluted":true}}'); expect(o).toBeDefined();`],
  ['traversal-filename', `it('../../../../etc/passwd', () => { expect(1).toBe(1); });`],
  ['null-bytes', `it('x\x00y', () => { expect(1).toBe(1); });`],
  ['deep-nesting', `it('x', () => { ${'if(1){'.repeat(200)}}${'}'.repeat(200)} });`],
  ['regex-dos', `it('x', () => { expect(new RegExp('(a+)+$').test('aaaaaaaaaaaaaaaaaaaaaaaaaaaa!')).toBe(false); });`],
  ['empty-file', ''],
  ['only-whitespace', '   \n\t\n   '],
  ['binary-garbage', '\x00\x01\x02\xFF\xFE\xFD it(x expect'],
  ['unicode-directional', "it('\u202Etest\u202D', () => { expect(1).toBe(1); });"],
  ['huge-single-line', `it('x', () => { expect('${'a'.repeat(100_000)}').toHaveLength(100000); });`],
];
  ...
  for (const [name, snippet] of HOSTILE_SNIPPETS) {
    it(`survives: ${name}`, () => {
      for (const rule of RULES) {
        expect(() => rule.run({ path: 'evil.spec.ts', text: snippet }), `${rule.id} on ${name}`).not.toThrow();
      }
    });
  }

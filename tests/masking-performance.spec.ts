...
describe("P-1 re-check: masking scales linearly, 1 MB within budget", () => {
  ...
  // (npm run test:coverage slows string-heavy masking ~3x and CI runners
  // vary further) — the 1 MB budget asserts WALL-CLOCK performance, which
  // the coverage job cannot measure honestly. The plain `npm test` runs
  // on every PR and every platform keep the guarantee.
  ...
  it.skipIf(underCoverage)(
    "masks a 1 MB file well under the per-file analysis budget",
    () => {
  ...
  it.skipIf(underCoverage)(
    "doubling the size does not more than double the time (linear, not quadratic)",
    () => {

...
describe("marker constants", () => {
  it("UNIFIED_MARKER has the v2 comment marker", () => {
    expect(UNIFIED_MARKER).toBe("<!-- qa-doctor-report:v2 -->");
  });

  it("LEGACY_PR_MARKER has the legacy pr comment marker", () => {
    expect(LEGACY_PR_MARKER).toBe("<!-- qa-doctor-pr-comment -->");
  });

  it("LEGACY_TRUST_MARKER has the legacy trust report marker", () => {
    expect(LEGACY_TRUST_MARKER).toBe("<!-- qa-doctor-trust-report:v1 -->");
  });
});
...
describe("renderUnifiedReport", () => {
  it("starts with the unified marker", () => {
    ...
    expect(out).toMatch(/^<!-- qa-doctor-report:v2 -->/);
  });

  it("renders hero header with qa-doctor icon", () => {
    const out = renderUnifiedReport(result());
    expect(out).toContain("QA Doctor Verification Report");
  ...
  it("truncates findings at 25 per group and shows overflow", () => {
    ...
    expect(out).toContain("Run `qa-doctor` locally");
  });
  ...
  it("renders default repo URL", () => {
    const out = renderUnifiedReport(result());
    expect(out).toContain("https://github.com/Sergey-Bar/qa-doctor");
  });
  ...
  it("renders custom repo URL in footer", () => {
    ...
    expect(out).toContain(
      "Generated by [QA Doctor](https://github.com/custom/repo)",
    );
  });

 ...
 *
 * Requires npm >= 10.9 (for `npm sbom --sbom-format spdx`).
 * Falls back to a minimal hand-built SPDX document if the installed
 * npm does not support the `sbom` subcommand.
 */
...
function tryNativeSbom() {
  try {
    const output = execSync("npm sbom --sbom-format spdx --json", {
      encoding: "utf8",
...
if (native) {
  ...
  const sbom = buildMinimalSpdx(pkg);
